...
<div class="w-full flex justify-center items-center min-h-[60vh] bg-gradient-to-br from-blue-50 to-blue-200 py-8">
    <div class="bg-white/90 backdrop-blur rounded-2xl shadow-2xl p-10 max-w-4xl w-full">
        <h2 class="text-2xl font-extrabold text-blue-900 mb-6 tracking-wide">Добавить страницу</h2>
        ...
        <form method="POST" action="{{ route('pages.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" value="{{ old('title') }}" required class="w-full border-2 border-blue-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>
            <div>
                <label for="slug" class="block text-gray-700 font-semibold mb-1">Slug (URL)</label>
                <input type="text" id="slug" name="slug" value="{{ old('slug') }}" required class="w-full border-2 border-blue-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="about-us">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" id="meta_title" name="meta_title" value="{{ old('meta_title') }}" class="w-full border-2 border-blue-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>
            <div>
                <label for="meta_description" class="block text-gray-700 font-semibold mb-1">Meta описание (опционально)</label>
                <textarea id="meta_description" name="meta_description" rows="3" class="w-full border-2 border-blue-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('meta_description') }}</textarea>
            </div>
            <div class="flex items-center">
                <input type="checkbox" id="is_published" name="is_published" value="1" checked class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
