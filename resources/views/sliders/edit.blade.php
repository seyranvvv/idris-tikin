...
<div class="w-full flex justify-center items-center min-h-[60vh] bg-gradient-to-br from-blue-50 to-blue-200 py-8">
    <div class="bg-white/90 backdrop-blur rounded-2xl shadow-2xl p-10 max-w-lg w-full">
        <h2 class="text-2xl font-extrabold text-blue-900 mb-6 tracking-wide">Редактировать слайдер</h2>
        ...
        <form method="POST" action="{{ route('sliders.update', $slider->id) }}" class="space-y-5" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div>
                <label for="name" class="block text-gray-700 font-semibold mb-1">Название</label>
                <input type="text" id="name" name="name" value="{{ old('name', $slider->name) }}" required class="w-full border-2 border-blue-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>
            <div>
                <label for="link" class="block text-gray-700 font-semibold mb-1">Ссылка (опционально)</label>
                <input type="url" id="link" name="link" value="{{ old('link', $slider->link) }}" class="w-full border-2 border-blue-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="https://example.com">
            </div>
            <div>
                <label for="image" class="block text-gray-700 font-semibold mb-1">Изображение</label>
                @if($slider->image)
                    <img src="{{ asset('storage/' . $slider->image) }}" alt="current" class="h-20 w-32 object-cover rounded-lg border mb-2" />
                @endif
